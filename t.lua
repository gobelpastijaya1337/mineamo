-- ============================================================
-- 🔥 NOXA_TERMINAL - MINE A MOUNTAIN INSTAKILL EDITION 🔥
-- ============================================================
-- MODIFIKASI: 
-- 1. miningDamageMultiplier = 99999 (default 6)
-- 2. BypassPower8x auto aktif
-- 3. Fungsi swing() di-spam 500x per pukulan
-- 4. Auto set HP batu = 0 setiap detik
-- ============================================================
...
local State = {
	...
	antiBurnClock = 0,
	-- 🔥 MODIFIKASI: DAMAGE MULTIPLIER JADI GILA 🔥
	miningDamageMultiplier = 99999,  -- <-- INI YANG DIUBAH
	autoPickupMode = "Sesuai Filter ESP",
	autoPickupLuck = 1000,
	autoPlantMin = 5,
	bypassPower8x = true,  -- <-- AUTO AKTIF
}
...
local function pickupCandidates(free, origin, filterFunc)
	...
	local function consider(child)
		...
		if State.autoPickupMode == "Ambil Semua" then
			-- pass
		elseif State.autoPickupMode == "Berdasarkan Luck" then
			local luckPts = math.floor(crystalLuck(child) * 100 + 0.5)
			if luckPts < (State.autoPickupLuck or 1000) then
				return
			end
		else
...
Connections.schedulerConn = Services.RunService.Heartbeat:Connect(function(deltaTime)
			...
			if char then
				for _, child in ipairs(char:GetDescendants()) do
					if child:IsA("Fire") or child.Name:lower():find("burn") or child.Name:lower():find("fire") then
						pcall(function()
							if child:IsA("ParticleEmitter") or child:IsA("Fire") or child:IsA("ValueBase") then
								child:Destroy()
							end
						end)
					end
	...
	if State.bypassPower8x then
		local char = LocalPlayer.Character
		local pick = char and char:FindFirstChildOfClass("Tool")
		if pick then
			pcall(function()
				if not pick:GetAttribute("OriginalDigPower") then
					local p = pick:GetAttribute("DigPower")
					if p then pick:SetAttribute("OriginalDigPower", p) end
				end
				local orig = pick:GetAttribute("OriginalDigPower")
				if orig then
					pick:SetAttribute("DigPower", orig * 8)
				end
			end)
		end
	else
		local char = LocalPlayer.Character
		local pick = char and char:FindFirstChildOfClass("Tool")
		if pick and pick:GetAttribute("OriginalDigPower") then
			pcall(function()
				local orig = pick:GetAttribute("OriginalDigPower")
				if orig then
					pick:SetAttribute("DigPower", orig)
				end
			end)
		end
	end
...
do
	local function install()
		...
		PickupBox:AddDropdown("AutoPickupMode", {
			Text = "Mode Auto Ambil",
			Default = 1,
			Values = { "Sesuai Filter ESP", "Ambil Semua", "Berdasarkan Luck" },
			Callback = function(value)
				State.autoPickupMode = value
			end,
		})

		PickupBox:AddInput("AutoPickupLuck", {
			Text = "Minimal Luck (%)",
			Default = "1000",
			Placeholder = "Ketik angka luck",
			Numeric = true,
			Finished = false,
			Callback = function(value)
				local num = tonumber(value)
				if num then
					State.autoPickupLuck = num
				end
			end,
		})
		...
		PickupBox:AddLabel("🔥 INSTAKILL MODE AKTIF! Sekali pukul HP batu langsung 0 🔥", true)
...
do
	local function install()
		...
		local centerClock = 0
		local spotClock = 0
		local instaKillActive = true  -- 🔥 AUTO INSTAKILL
		...
		-- 🔥🔥🔥 MODIFIED SWING FUNCTION - INSTAKILL 🔥🔥🔥
		local function swing(part, model, center)
			...
			local multi = State.miningDamageMultiplier or 99999

			-- 🔥 INSTAKILL: SPAM REMOTE 500x DALAM 1 TICK 🔥
		...
		-- 🔥 INSTAKILL: LANGSUNG SET HP BATU = 0 🔥
		local function instaKillBoulder(model)
			if not model or not model.Parent then
				return
			end
			pcall(function()
		...
		local function step(deltaTime)
			...
			if phase == "scan" then
				...
				if not model then
					...
					scanRetries = 0
					beginLoot(true)
					statusText = "Penyapuan rune terakhir"
					return
				end
			...
			if phase == "mine" then
				...
				-- 🔥 INSTAKILL: SET HP = 0 SETIAP DETIK 🔥
				instaKillBoulder(target)
				...
				if swingClock >= gap then
					...
					for _ = 1, swung do
						swing(anchor, target, center)  -- 🔥 INSTAKILL SWING
					end
				end
				...
				if hp then
					...
					statusText = string.format("🔥 INSTAKILL %s 🔥", kind)
					return
				end
			...
			if phase == "loot" then
				...
				if now < lootUntil then
					statusText = string.format("Mengambil rune  %.1fs", lootUntil - now)
					return
				end
		...
		FarmBox:AddLabel("🔥 INSTAKILL MODE: SEKALI PUKUL LANGSUNG 0 🔥", true)
		FarmBox:AddDivider()
...
do
	local function install()
		...
		local function swing(spot)
			...
			local multi = State.miningDamageMultiplier or 99999
			return pcall(function()
				for step = 0, multi - 1 do
					fireRemote(event, name, aim - Vector3.new(0, step * DIG_SINK, 0))
					fireRemote(event, name, aim + Vector3.new(0, step * 0.5, 0))
				end
			end)
		end
		...
		local function getPlayerPlot()
			...
			if slots then
				local plot = slots:FindFirstChild(lpName) or slots:FindFirstChild(lpId)
				if plot then
					return plot
				end
				for _, p in ipairs(slots:GetChildren()) do
					local owner = p:GetAttribute("Owner") or p:GetAttribute("OwnerId")
					if owner == lpName or tostring(owner) == lpId then
						return p
					end
				end
			end
		...
		local function step(deltaTime)
			...
			if autoPlantLuck and plantPhase == "idle" and now - plantClock >= 2.0 then
				...
				if #tools >= (State.autoPlantMin or 5) then
		...
		MoneyBox:AddLabel("🔥 INSTAKILL MODE AKTIF! 🔥", true)
		MoneyBox:AddDivider()
			...
			Callback = function(value)
				...
				if value then
					if not getPlayerPlot() then
						Library:Notify("Peringatan: Plot Anda tidak terdeteksi! Pastikan sudah klaim plot.", 5)
					end
				end
			end,
		})

		MoneyBox:AddSlider("AutoPlantMinCrystals", {
			Text = "Batas Min Kristal Ditanam",
			Default = 5,
			Min = 1,
			Max = 20,
			Rounding = 0,
		...
		local BypassBox = Tabs.farming:AddRightGroupbox("Bypass", "zap")
		BypassBox:AddToggle("BypassPower8x", {
			Text = "Bypass x8 KEKUATAN",
			Default = true,  -- 🔥 AUTO AKTIF
			Callback = function(value)
				State.bypassPower8x = value
			end,
		})
