...
do
	local function install()
		...
		local function swing(part, model, center)
			...
			local sweepList = {}
			local multi = State.miningDamageMultiplier or 6
			if model and model.Parent then
				local list = partList(model)
				local count = #list

				if count > 0 then
					local sweepCount = math.min(math.max(SWEEP_PARTS, math.floor(multi / 2)), count)
					for _ = 1, sweepCount do
						partCursor = partCursor % count + 1
						local item = list[partCursor]
						if item and item.Parent then
			...
			return pcall(function()
				...
				for _, point in ipairs(sweepList) do
					for _ = 1, math.max(1, math.floor(multi / 5)) do
		...
		FarmBox:AddLabel("Dikhususkan untuk farming rune", true)
		FarmBox:AddDivider()
