...
do
	local function install()
		...
		local BypassGroup = SettingsTab:AddRightGroupbox("Bypass Gamepass", "unlock")

		BypassGroup:AddInput("GamepassName", {
			Text = "Nama Gamepass",
			Default = "x8",
			Placeholder = "Contoh: x8Luck",
			Numeric = false,
			Finished = false,
			Callback = function() end,
		})

		BypassGroup:AddButton("Aktifkan Bypass (Lokal)", function()
			local name = Library.Options.GamepassName.Value
			if name and name ~= "" then
				local folder = LocalPlayer:FindFirstChild("GamepassesOwned")
				if not folder then
					folder = Instance.new("Folder")
					folder.Name = "GamepassesOwned"
				...
				val.Value = true
				Library:Notify("Gamepass '" .. name .. "' berhasil diaktifkan!", 3)
			else
				Library:Notify("Nama gamepass tidak boleh kosong!", 2)
			end
		end)
